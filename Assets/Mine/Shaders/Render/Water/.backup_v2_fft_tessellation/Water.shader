Shader "Custom/Water"
{
    ...
    // ════════════════════════════════════════════════════════════
    //  ComputeWaveFoam — 波峰泡沫
    // ════════════════════════════════════════════════════════════
    float ComputeWaveFoam(float3 positionWS)
    {
        float timeSpeed = _Time.y * _FoamSpeed;
        float2 baseUV = positionWS.xz * _FoamScale + timeSpeed * 0.05;
        baseUV *= 0.5;

        float foam0 = 1.0 - _FoamTex.Sample(sampler_FoamTex, baseUV).g;
        float foam1 = 1.0 - _FoamTex.Sample(sampler_FoamTex, baseUV * 2.0 + float2(0.13, 0.27) * timeSpeed).g;
        float foam2 = 1.0 - _FoamTex.Sample(sampler_FoamTex, baseUV * 4.0 + float2(-0.31, 0.19) * timeSpeed).g;
        float foam3 = 1.0 - _FoamTex.Sample(sampler_FoamTex, baseUV * 8.0 + float2(0.23, -0.17) * timeSpeed).g;
    ...
    float3 ComputeCaustics(float3 positionWS, float3 mainLitDir, float3 scenePosWS, float sceneDepDf, float3 sceneNorWS)
    {
        ...
        float colorG = _FoamTex.Sample(sampler_FoamTex, surfaceHit.xz  * _FoamScale + _Time.y * _FoamSpeed * 0.1).g;
