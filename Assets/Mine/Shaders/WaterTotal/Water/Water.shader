Shader "Custom/Water"
{
    ...
    float3 ComputeOpaque(float3 baseColor, float3 positionWS, float3 normalWS, float3 viewDirWS, float3 mainLitDir, float3 mainLitColor, float mainLitDistanceAtten, float mainLitShadowAtten)
    {
        ...
        float3 diffuse  = Diff_Lambert(baseColor) * PI * radiance * ndotl * (1.0 - metallic) * (1.0 - F);
    ...
    {
        float2 baseUV = positionWS.xz * _FoamScale + _Time.y * _FoamSpeed * 0.05;
        baseUV *= 0.5;

        float foam0 = 1 - _FoamTex.Sample(sampler_FoamTex, baseUV).g;
        float foam1 = 1 - _FoamTex.Sample(sampler_FoamTex, baseUV * 2.0 + float2(0.13, 0.27) * (_Time.y * _FoamSpeed)).g;
        float foam2 = 1 - _FoamTex.Sample(sampler_FoamTex, baseUV * 4.0 + float2(-0.31, 0.19) * (_Time.y * _FoamSpeed)).g;
        float foam3 = 1 - _FoamTex.Sample(sampler_FoamTex, baseUV * 8.0 + float2(0.23, -0.17) * (_Time.y * _FoamSpeed)).g;
        ...
        waveFoam = smoothstep(0.4, 1, waveFoam * _FoamIntensity);
        return waveFoam;
    }
